...
void CollisionChecker::check(Particle& p, Obstacle& o) {
    if (auto solidRect = dynamic_cast<SolidRectObstacle*>(&o)) {
        check(p, *solidRect);
    } else if (auto hollowRect = dynamic_cast<HollowRectObstacle*>(&o)) {
        check(p, *hollowRect);
...
void CollisionChecker::check(Particle& p, HollowRectObstacle& r) {
    double cosA = std::cos(r.rotation);
    double sinA = std::sin(r.rotation);

    double halfW = r.width / 2.0;
    double halfH = r.height / 2.0;
    double halfT = r.thickness / 2.0;

    // Helper lambda: check particle against a thin rectangle (centered at cx, cy, w, h, rotated)
    ...
    // Top wall
    checkWall(r.x, r.y - halfH + halfT, r.width, r.thickness);
    // Bottom wall
    checkWall(r.x, r.y + halfH - halfT, r.width, r.thickness);
    // Left wall
    checkWall(r.x - halfW + halfT, r.y, r.thickness, r.height);
    // Right wall
    checkWall(r.x + halfW - halfT, r.y, r.thickness, r.height);
}
